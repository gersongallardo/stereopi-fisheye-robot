...
# PLY writers.
# ---------------------------------------------------------------------------
def save_points_to_ply(sdk: OrbbecSDK, frame: ctypes.c_void_p, path: Path) -> None:
    ...
    with path.open("w", encoding="utf-8") as ply:
        ply.write("ply\nformat ascii 1.0\n")
        ply.write(f"element vertex {count}\n")
        ply.write("property float x\nproperty float y\nproperty float z\n")
        ply.write("end_header\n")
        for idx in range(count):
            point = point_array[idx]
            ply.write(f"{point.x:.3f} {point.y:.3f} {point.z:.3f}\n")


def save_rgb_points_to_ply(sdk: OrbbecSDK, frame: ctypes.c_void_p, path: Path) -> None:
    data_size = sdk.call(sdk.lib.ob_frame_data_size, frame)
    ...
    with path.open("w", encoding="utf-8") as ply:
        ...
        for idx in range(count):
            ...
            ply.write(
                ...
                f"{int(point.r)} {int(point.g)} {int(point.b)}\n"
            )
    ...
    while attempts < max_attempts:
        ...
        if points_frame:
            if point_format == OB_FORMAT_RGB_POINT:
                save_rgb_points_to_ply(sdk, points_frame, destination)
            else:
                save_points_to_ply(sdk, points_frame, destination)
