...
class DepthDiveController:
    ...
    def _set_pwm(self, us):
        us = self._clamp_us(us)
        self._current_pwm = us
    ...
    def _log_sample(self, state, sample, stagnation_age):
        ...
        self._csv_writer.writerow(
            [
                ...
                state,
                f"{sample['depth']:.4f}",
                f"{sample['pressure_mbar']:.2f}",
                f"{sample['temperature_c']:.3f}",
                f"{sample['altitude_m']:.3f}",
                self._current_pwm,
                f"{stagnation_age:.2f}",
            ]
        )
    ...
    def run(self):
        ...
        if self._csv_writer:
            print(f"[LOG] Registrando muestras en: {self.log_path}")

        try:
            while self.running:
                sample = self._read_full()
                depth = sample["depth"]
                stagnation_age = self._update_stagnation(depth)

                print(
                    f"[{state}] Profundidad: {depth:.3f} m | "
                    f"Δt sin cambio: {stagnation_age:.1f}s | PWM: {self._current_pwm}us"
                )
                self._log_sample(state, sample, stagnation_age)
...
def parse_args():
    ...
    ap.add_argument("--sample-period", type=float, default=0.5, help="Segundos entre lecturas del sensor.")
    ap.add_argument(
        "--stagnation-timeout",
        type=float,
        default=20.0,
    ...
    ap.add_argument(
        "--log-file",
        type=str,
        default=os.path.join(CURRENT_DIR, "depth_log.csv"),
        help="Ruta de archivo CSV para registrar muestras. Usa '' para desactivar.",
    )
