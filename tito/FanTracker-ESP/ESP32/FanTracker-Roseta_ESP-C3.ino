...
// ===== WiFi (SoftAP) =====
const char* AP_SSID = "FanTracker-ESP32C3";
...
// ===== Batería =====
const int BATTERY_PIN = 2; // GPIO2 (ADC1) libre en ESP32-C3 
const float ADC_REF_V = 3.3;
const float BATTERY_DIVIDER_RATIO = 6.0; // (100k + 20k) / 20k
const float BATTERY_CALIBRATION = 0.997374557; // Ajuste según multímetro
...
const int BATTERY_CMD_SAMPLES = 7;
const int BATTERY_CMD_TRIM = 1;
const float BATTERY_CUTOFF_PERCENT_DEFAULT = 40.0;
float batteryCutoffPercent = BATTERY_CUTOFF_PERCENT_DEFAULT;
bool batteryLockout = false;

// ===== Motor T200 =====
const int MOTOR_PIN = 5;  // GPIO para PWM del ESC
...
float descentPWM_us = 1670;  // PWM para descender
...
float lastDepth = 0.0;
unsigned long lastValidDepthSampleMs = 0;
unsigned long stagnationAccumulatedMs = 0;
...
unsigned long autoPreStartMs = 60000;
bool autoSleepEnabled = true;
String missionEndReason = "";
bool sleepPending = false;
unsigned long sleepRequestMs = 0;
unsigned long missionSleepThresholdMs = 1800000;
unsigned long postMissionSleepDelayMs = 300000;
unsigned long oxyOffBeforeSleepMs = 30000;

// ===== Oxy controller (MOSFET) =====
const int OXY_GATE_PIN = 18;
bool oxyEnabled = false;
bool oxyOffScheduled = false;
...
enum BatteryReadMethod { BATTERY_METHOD_TRIMMED_MEAN, BATTERY_METHOD_TRIMMED_MEDIAN };
BatteryReadMethod batteryReadMethod = BATTERY_METHOD_TRIMMED_MEAN;
...
void setOxySwitch(bool on) {
  oxyEnabled = on;
  if (on) {
    pinMode(OXY_GATE_PIN, OUTPUT);
    digitalWrite(OXY_GATE_PIN, LOW);
    sendLine("[OXY] ON");
  } else {
    pinMode(OXY_GATE_PIN, INPUT);
    sendLine("[OXY] OFF");
  }
}
...
int getNextLogIndex() {
  ...
  while (file) {
    ...
    if (name.startsWith("/log_") && name.endsWith(".csv")) {
      String indexStr = name.substring(5, name.length() - 4);
      if (isAllDigits(indexStr)) {
...
String readBar30CSV() {
  if (!readSensorFiltered()) {
    ...
    float depthRelative = missionZeroValid ? (lastIgnoredDepth - missionZeroDepth) : 0.0f;
    String line;
    line.reserve(140);
    line += getTimestamp();
    line += ",error,";
  ...
  float batteryVoltage = readBatteryVoltage();
  float batteryPercent = batteryPercentFromVoltage(batteryVoltage);
  float depthRelative = missionZeroValid ? (lastValidDepth - missionZeroDepth) : 0.0f;
  String line;
  line.reserve(140);
  line += getTimestamp();
  line += ",";
...
float readBatteryVoltage() {
  uint32_t adcMilliVoltsSum = 0;
  for (int i = 0; i < BATTERY_SAMPLES; i++) {
    adcMilliVoltsSum += analogReadMilliVolts(BATTERY_PIN);
    delayMicroseconds(250);
  }
  float v_adc = (adcMilliVoltsSum / (float)BATTERY_SAMPLES) / 1000.0f;
  return v_adc * BATTERY_DIVIDER_RATIO * BATTERY_CALIBRATION;
}

float batteryPercentFromVoltage(float voltage) {
  // Curva aproximada para Li-Ion/LiPo 4S en reposo.
  // Mejora la estimación en la parte baja, donde una regla lineal suele subestimar.
  static const float voltageLut[] = {
    12.00, 12.40, 12.60, 12.80, 13.00, 13.20, 13.40, 13.60,
    13.80, 14.00, 14.20, 14.40, 14.60, 14.80, 15.20, 15.60,
    16.00, 16.40, 16.80
  };
  static const float percentLut[] = {
      0.0,   8.0,  15.0,  22.0,  30.0,  38.0,  46.0,  54.0,
     62.0,  70.0,  76.0,  82.0,  87.0,  91.0,  95.0,  97.0,
     98.5,  99.5, 100.0
  };

  if (voltage <= voltageLut[0]) return 0.0f;
  int last = (sizeof(voltageLut) / sizeof(voltageLut[0])) - 1;
  if (voltage >= voltageLut[last]) return 100.0f;

  for (int i = 1; i <= last; i++) {
    if (voltage <= voltageLut[i]) {
      float v0 = voltageLut[i - 1];
      float v1 = voltageLut[i];
      float p0 = percentLut[i - 1];
...
const char* getBatteryMethodName(BatteryReadMethod method) {
  switch (method) {
    case BATTERY_METHOD_TRIMMED_MEAN: return "trimmed_mean";
    case BATTERY_METHOD_TRIMMED_MEDIAN: return "trimmed_median";
    default: return "unknown";
  }
}

float readBatteryVoltageFiltered(BatteryReadMethod method) {
  float samples[BATTERY_CMD_SAMPLES];
  for (int i = 0; i < BATTERY_CMD_SAMPLES; i++) {
    samples[i] = readBatteryVoltage();
    delay(8);
  }

  for (int i = 0; i < BATTERY_CMD_SAMPLES - 1; i++) {
    for (int j = i + 1; j < BATTERY_CMD_SAMPLES; j++) {
  ...
  int start = BATTERY_CMD_TRIM;
  int end = BATTERY_CMD_SAMPLES - BATTERY_CMD_TRIM;
  int count = end - start;
  if (count <= 0) {
    return readBatteryVoltage();
  }

  if (method == BATTERY_METHOD_TRIMMED_MEDIAN) {
...
void writeLogHeader() {
  ...
  logFile.println("sleep_threshold_s=" + String(missionSleepThresholdMs / 1000.0, 1) + " s");
  logFile.println("sleep_delay_s=" + String(postMissionSleepDelayMs / 1000.0, 1) + " s");
  logFile.println("=== BEGIN FILE: " + currentLogFile + " ===");
  logFile.println("oxy_prestart_s=" + String(autoPreStartMs / 1000.0, 1) + " s");
  logFile.println("oxy_off_before_sleep_s=" + String(oxyOffBeforeSleepMs / 1000.0, 1) + " s");
  logFile.println("auto_sleep_enabled=" + String(autoSleepEnabled ? "1" : "0"));
  if (readSensorFiltered()) {
    logFile.println("depth_zero_init_m=" + String(missionZeroDepth, 3));
    logFile.println("depth_init_m=" + String(lastValidDepth, 3));
    logFile.println("pressure_init_mbar=" + String(lastValidPressure, 2));
  }
  logFile.println("timestamp,state,pressure_mbar,temp_C,depth_m,battery_v,battery_pct");
}
...
void startLogging() {
  ...
  currentLogFile = "/log_" + String(nextIndex) + ".csv";
...
void requestDeepSleepNow(const String& reason) {
  sleepPending = true;
  sleepRequestMs = millis() - postMissionSleepDelayMs;
  sendLine("[SLEEP] " + reason + ". Entrando en deep sleep...");
}
...
void startMission(float depth_m) {
  ...
  if (batteryLockout) {
    ...
    requestDeepSleepNow("Bloqueo por batería baja");
    return;
  }
  ...
  stagnationAccumulatedMs = 0;
  lastValidDepthSampleMs = missionStartTime;
  if (readSensorFiltered()) {
    lastDepth = lastValidDepth;
  } else {
    lastDepth = 0.0f;
  }
  lastDepthChangeTime = missionStartTime;
  ...
  if (batteryPercent < batteryCutoffPercent) {
    sendLine("[BATERÍA] < " + String(batteryCutoffPercent, 1) + "%. Auto OFF y misión cancelada.");
    ...
    missionState = IDLE;
    requestDeepSleepNow("Batería por debajo del cutoff");
    return;
  }
...
void updateStagnationDetection(float depth) {
  unsigned long now = millis();
  if (lastValidDepthSampleMs == 0) {
    lastValidDepthSampleMs = now;
  }
  stagnationAccumulatedMs += (now - lastValidDepthSampleMs);
  lastValidDepthSampleMs = now;

  if (fabsf(depth - lastDepth) >= stagnationDelta) {
...
void processMission() {
  ...
  unsigned long now = millis();
  float stagnationAge = stagnationAccumulatedMs / 1000.0f;
  ...
  switch(missionState) {
    ...
    case ASCENDING:
      ...
      if (depth <= surfaceBand) {
        ...
        bool shouldSleep = autoSleepEnabled && autoMissionEnabled && autoMissionIntervalMs >= missionSleepThresholdMs;
        if (!shouldSleep && missionDurationMs >= missionSleepThresholdMs) {
          shouldSleep = true;
        }
        if (shouldSleep) {
          sleepPending = true;
          sleepRequestMs = millis();
          oxyOffScheduled = false;
          sendLine("[SLEEP] Entrando en deep sleep en " +
                   String(postMissionSleepDelayMs / 1000.0, 0) + "s.");
        }
...
void processAutoMission() {
  ...
  unsigned long now = millis();
  if (nextAutoMissionMs > autoPreStartMs && now >= (nextAutoMissionMs - autoPreStartMs) && !oxyEnabled) {
    setOxySwitch(true);
  }
...
void handleCommand(const String& cmdRaw) {
  ...
  if (cmd == "help") {
    ...
    client.println("  oxy on|off    - encender/apagar Oxy controller");
    ...
    client.println("            auto_interval, auto_prestart, sleep_threshold, sleep_delay,");
    client.println("            oxy_sleep_off, auto_sleep, battery_cutoff, battery_method");
  ...
  if (cmd == "oxy on") { setOxySwitch(true); client.println("oxy=ON"); return; }
  if (cmd == "oxy off") { setOxySwitch(false); client.println("oxy=OFF"); return; }
  ...
  if (cmd == "battery") {
    float batteryVoltage = readBatteryVoltageFiltered(batteryReadMethod);
    float batteryPercent = batteryPercentFromVoltage(batteryVoltage);
    client.println("battery_v=" + String(batteryVoltage, 2) +
                   " V | battery_pct=" + String(batteryPercent, 1) +
                   "% | method=" + String(getBatteryMethodName(batteryReadMethod)) +
                   "(" + String(BATTERY_CMD_SAMPLES) + ",trim=" + String(BATTERY_CMD_TRIM) + ")");
    return;
  }
  ...
  if (cmd == "status") {
    ...
    client.println("Auto prestart: " + String(autoPreStartMs / 1000.0, 1) + " s");
    client.println("Sleep threshold: " + String(missionSleepThresholdMs / 1000.0, 1) + " s");
    client.println("Sleep delay: " + String(postMissionSleepDelayMs / 1000.0, 1) + " s");
    client.println("Oxy off before sleep: " + String(oxyOffBeforeSleepMs / 1000.0, 1) + " s");
    client.println("Auto sleep: " + String(autoSleepEnabled ? "on" : "off"));
    client.println("Oxy state: " + String(oxyEnabled ? "on" : "off"));
    return;
  }
  ...
  if (cmd.startsWith("config ")) {
    ...
    if (spaceIdx > 0) {
      ...
      } else if (param == "auto_prestart") {
        if (value < 0) value = 0;
        autoPreStartMs = (unsigned long)(value * 1000.0f);
        client.println("auto_prestart=" + String(autoPreStartMs / 1000.0, 1));
      } else if (param == "sleep_threshold") {
        if (value < 0) value = 0;
        missionSleepThresholdMs = (unsigned long)(value * 1000.0f);
        client.println("sleep_threshold=" + String(missionSleepThresholdMs / 1000.0, 1));
      } else if (param == "sleep_delay") {
        if (value < 5) value = 5;
        postMissionSleepDelayMs = (unsigned long)(value * 1000.0f);
        client.println("sleep_delay=" + String(postMissionSleepDelayMs / 1000.0, 1));
      } else if (param == "oxy_sleep_off") {
        if (value < 0) value = 0;
        oxyOffBeforeSleepMs = (unsigned long)(value * 1000.0f);
        client.println("oxy_sleep_off=" + String(oxyOffBeforeSleepMs / 1000.0, 1));
      } else if (param == "auto_sleep") {
        autoSleepEnabled = (value > 0.5f);
        client.println("auto_sleep=" + String(autoSleepEnabled ? "1" : "0"));
      } else if (param == "battery_cutoff") {
        batteryCutoffPercent = constrain(value, 0.0f, 100.0f);
        client.println("battery_cutoff=" + String(batteryCutoffPercent, 1));
      } else if (param == "battery_method") {
        if (valueStr == "trimmed_mean" || valueStr == "mean" || valueStr == "0") {
          batteryReadMethod = BATTERY_METHOD_TRIMMED_MEAN;
        } else if (valueStr == "trimmed_median" || valueStr == "median" || valueStr == "1") {
          batteryReadMethod = BATTERY_METHOD_TRIMMED_MEDIAN;
        } else {
          client.println("ERROR: battery_method inválido. Usa mean|median (o 0|1)");
          return;
        }
        client.println("battery_method=" + String(getBatteryMethodName(batteryReadMethod)));
      } else {
...
void loop() {
  ...
  if (sleepPending) {
    ...
    if (!oxyOffScheduled && oxyEnabled && postMissionSleepDelayMs > oxyOffBeforeSleepMs &&
        elapsed >= (postMissionSleepDelayMs - oxyOffBeforeSleepMs)) {
      setOxySwitch(false);
      oxyOffScheduled = true;
    }
    if (elapsed >= postMissionSleepDelayMs) {
      if (loggingEnabled) {
        stopLogging();
      }
