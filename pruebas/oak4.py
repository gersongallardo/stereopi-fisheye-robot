...
try:
    with dai.Device(pipeline) as device:
        ...
        logger.info("Capturando nube de puntos, imagen infrarroja y vista de profundidad...")
        ...
        try:
            depth_frame = q_depth.get().getFrame().astype(np.float32)
            depth_frame[depth_frame == 0] = np.nan

            # Normalizar para vista previa
            valid_mask = ~np.isnan(depth_frame)
            if np.any(valid_mask):
                min_depth = np.nanmin(depth_frame)
                max_depth = np.nanmax(depth_frame)
            else:
                min_depth = 0
                max_depth = 1

            depth_preview = depth_frame.copy()
            depth_preview[~valid_mask] = min_depth

            depth_preview = cv2.normalize(depth_preview, None, 0, 255, cv2.NORM_MINMAX)
            depth_preview = depth_preview.astype(np.uint8)
            depth_preview = cv2.applyColorMap(depth_preview, cv2.COLORMAP_TURBO)

            depth_filename = f"{filename}_depth_preview.png"
            if cv2.imwrite(f'{data_directory}/{depth_filename}', depth_preview):
                logger.info("[Depth Preview] Guardado como: %s", depth_filename)
            else:
                logger.error("No se pudo guardar la vista de profundidad: %s", depth_filename)
        except Exception:
            logger.exception("Fallo al capturar o guardar la vista de profundidad")
