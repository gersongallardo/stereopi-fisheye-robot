...
cubo.apply_translation((0, 0, altura_prisma + silo.alto_piramide + silo.alto_ortoedro / 2))
...
piramide = crear_piramide_truncada(silo.largo, silo.largo, silo.ancho_orificio, silo.largo_orificio, silo.alto_piramide)
...
piramide.apply_translation([0, 0, silo.alto_piramide])


def crear_prisma_trapezoidal(base_menor, base_mayor, profundidad, altura):
    b_m = base_menor / 2
    b_M = base_mayor / 2
    p = profundidad / 2
    h = altura

    # Vértices: base inferior y superior
    v0 = [-b_m, -p, 0]
    v1 = [ b_m, -p, 0]
    v2 = [ b_m,  p, 0]
    v3 = [-b_m,  p, 0]
    ...
    # Crear un volumen convexo a partir de los puntos
    malla = trimesh.convex.convex_hull(vertices)

    return malla


# --- Crear prisma trapezoidal entre pirámide y ortoedro ---
prisma = crear_prisma_trapezoidal(silo.largo, silo.ancho,3.5, altura_prisma)

# Centrar y posicionar justo encima de la pirámide
# Base menor del trapecio conecta con la cima de la pirámide
prisma.apply_translation([0,0,
    silo.alto_piramide           # base inferior del prisma en la cima de la pirámide
])


def cortar_mitad(mesh, eje='x', lado='izquierdo', tolerancia=1e-5):
    """
...
def quitar_tapas(mesh, eje='z', tolerancia=1e-5):
    ...
    def es_tapa(face):
        valores = z_vals[face]
        return (np.allclose(valores, z_min, atol=tolerancia) or 
                np.allclose(valores, z_max, atol=tolerancia))

    caras_filtradas = np.array([face for face in faces if not es_tapa(face)])
    return trimesh.Trimesh(vertices=verts, faces=caras_filtradas, process=False)
...
# Visualización de silo final
mesh_silo_final= trimesh_to_open3d(silo_final)
o3d.visualization.draw_geometries([mesh_silo_final, axis, grid])

# Visualización de silo final sin tapas
o3d_mesh = trimesh_to_open3d(silo_sin_tapas)
o3d.visualization.draw_geometries([o3d_mesh, axis, grid])

# --- Guardar resultado ---
silo_sin_tapas.export(f'./silo_cam/labs/silos_mesh/{silo.empresa}-{silo.ubicacion}_{silo.silo}_{silo.sensor}.ply')
...
cubo_mitad = trimesh.creation.box(extents=(ancho, largo, silo.alto_piramide+altura_prisma))
cubo_mitad.apply_translation((0, 0, (silo.alto_piramide + altura_prisma)/2))  # base en z=0
