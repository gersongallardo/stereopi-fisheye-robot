...
float descentPWM_us = 1650;  // PWM para descender
float holdSeconds = 5.0;    // tiempo manteniendo profundidad
float armSeconds = 4.0;      // tiempo para armar ESC
float surfaceBand = 0.1;     // metros cerca de superficie
...
void handleCommand(const String& cmdRaw) {
  ...
  if (cmd == "help") {
    ...
    client.println("            stagnation_timeout, stagnation_delta");
  ...
  if (cmd.startsWith("config ")) {
    ...
    if (spaceIdx > 0) {
      ...
      if (param == "descent_pwm") {
        ...
        client.println("surface_band, stagnation_timeout, stagnation_delta");
      }
    } else {
