...
depth-only and RGBD point clouds. It can also capture a single infrared frame
and save it as a JPEG similar to the InfraredViewer sample.
...
#rsync -avz --progress pi@192.168.7.58:~/Orbbec/ ~/Descargas/Orbbec/
#en rpi voxel_size 0.05 pesa 140-95kb con 0.025 pesa 510-340kb
DEFAULT_OUTPUT_DIR = Path("/home/pi/Astra")
DEFAULT_LIBRARY_PATH = Path("/home/pi/OrbbecSDK/lib/arm64/libOrbbecSDK.so")
DEFAULT_SILO_ID = 1
DEFAULT_VOXEL_SIZE = 0.012
METERS_PER_UNIT = 0.001  # Astra depth coordinates are delivered in millimeters.
DEFAULT_IR_FILENAME = "{capture_id}_{timestamp}_ir.jpg"
...
class OrbbecSDK:
    ...
    def _configure_signatures(self) -> None:
        ...
        lib.ob_frameset_ir_frame.argtypes = [ctypes.c_void_p, err_ptr]
        lib.ob_frameset_ir_frame.restype = ctypes.c_void_p
        ...
        lib.ob_video_frame_width.argtypes = [ctypes.c_void_p, err_ptr]
        lib.ob_video_frame_width.restype = ctypes.c_uint32

        lib.ob_video_frame_height.argtypes = [ctypes.c_void_p, err_ptr]
...
def _rotate_axes(x: float, y: float, z: float) -> tuple[float, float, float]:
    """Mantiene X, invierte Y y Z y convierte a metros para orientación correcta."""
    # Mantiene X para evitar el efecto espejo
    # Invierte Y para que no esté de cabeza
    # Invierte Z para que esté de frente
    # Convierte de milímetros (unidad nativa del SDK) a metros
    return (
        x * METERS_PER_UNIT,
        -y * METERS_PER_UNIT,
        -z * METERS_PER_UNIT,
...
def save_infrared_frame(sdk: OrbbecSDK, frame: ctypes.c_void_p, path: Path) -> None:
    """Save infrared frame as JPEG with proper normalization for visibility."""
    lib = sdk.lib
    width = sdk.call(lib.ob_video_frame_width, frame)
    height = sdk.call(lib.ob_video_frame_height, frame)
    data_size = sdk.call(lib.ob_frame_data_size, frame)
    data_ptr = sdk.call(lib.ob_frame_data, frame)
    raw_bytes = ctypes.string_at(data_ptr, data_size)

    pixels = width * height
    if pixels <= 0:
        raise OrbbecError("Infrared frame returned zero-sized dimensions")

    bytes_per_pixel = max(1, data_size // pixels)
    
    # Convert raw bytes to array of pixel values
    if bytes_per_pixel == 1:
        # 8-bit data
        import array
        pixel_values = array.array('B', raw_bytes)
        pixel_array = list(pixel_values)
    else:
        # 16-bit data (Y16 format) - little endian
        import struct
        pixel_array = list(struct.unpack(f'<{pixels}H', raw_bytes))
    ...
    if pixel_array:
        ...
        if max_val > min_val:
            # Apply normalization with contrast stretch
            normalized = [
                int(255 * (val - min_val) / (max_val - min_val))
                for val in pixel_array
            ]
        else:
            normalized = [128] * len(pixel_array)  # All same value -> mid gray
    else:
        normalized = []
    ...
    image_array = np.array(normalized, dtype=np.uint8).reshape((height, width))

    path.parent.mkdir(parents=True, exist_ok=True)

    if not cv2.imwrite(str(path), image_array):
        raise OrbbecError(f"Failed to write infrared frame to {path}")
...
def configure_streams(
    ...
    ir_profiles = ir_profile = None
    if enable_infrared:
        ir_profiles = sdk.call(
            lib.ob_pipeline_get_stream_profile_list,
            pipeline,
            OB_SENSOR_IR,
            ignore_error=True,
        ...
        if ir_profiles:
            ...
            if ir_profile:
                sdk.call(lib.ob_config_enable_stream, config, ir_profile)
...
def capture_infrared(
    ...
    while attempts < max_attempts:
        ...
        if not frameset:
            attempts += 1
            continue
        ir_frame = sdk.call(lib.ob_frameset_ir_frame, frameset, ignore_error=True)
        sdk.call(lib.ob_delete_frame, frameset)
        if ir_frame:
            save_infrared_frame(sdk, ir_frame, destination)
            sdk.call(lib.ob_delete_frame, ir_frame)
            return True
        attempts += 1
    return False
...
def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "Capture a single infrared frame alongside the point clouds and write it as a JPEG "
    ...
    parser.add_argument(
        ...
        dest="capture_ir",
        action="store_false",
        help="Disable capturing the infrared frame.",
    )
    parser.add_argument(
        "--ir-filename",
        type=str,
        default=DEFAULT_IR_FILENAME,
        help=(
            "Filename template for the infrared capture. You can use {capture_id} and {timestamp} "
            "placeholders."
        ),
    )
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    try:
        (
            ...
            ir_profile,
            _align_mode,
        ) = configure_streams(sdk, pipeline, config, enable_infrared=args.capture_ir)
        ...
        if args.capture_ir:
            ir_filename = args.ir_filename.format(
                capture_id=args.capture_id, timestamp=timestamp
            )
            ir_destination = args.output_dir / ir_filename
            print(f"Capturing infrared frame to {ir_destination} ...")
            ok = capture_infrared(
                sdk,
                pipeline,
                args.timeout,
            ...
            if not ok:
                raise OrbbecError(
                    "Unable to capture an infrared frame within the configured attempts."
                )
            print(f"Saved {ir_destination}")
        ...
        if ir_profile:
            sdk.call(lib.ob_delete_stream_profile, ir_profile, ignore_error=True)
        if color_profiles:
        ...
        if ir_profiles:
            sdk.call(lib.ob_delete_stream_profile_list, ir_profiles, ignore_error=True)
