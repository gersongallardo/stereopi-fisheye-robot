...
and save it as a portable graymap (PGM) similar to the InfraredViewer sample.
...
DEFAULT_IR_FILENAME = "{capture_id}_{timestamp}_ir.pgm"
...
def save_infrared_frame(sdk: OrbbecSDK, frame: ctypes.c_void_p, path: Path) -> None:
    """Save infrared frame as PGM with proper normalization for visibility."""
    lib = sdk.lib
    ...
    # Write as 8-bit PGM (P5 format)
    header = f"P5\n{width} {height}\n255\n".encode("ascii")
    path.parent.mkdir(parents=True, exist_ok=True)
    
    with path.open("wb") as pgm:
        pgm.write(header)
        # Write normalized pixel data
        pgm.write(bytes(normalized))
...
def parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "Capture a single infrared frame alongside the point clouds and write it as a PGM "
