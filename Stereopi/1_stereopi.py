...
# ==================== CONFIGURACIÓN ====================
OUTPUT_DIR = Path("/home/pi/stereopi1")

# Ruta base del proyecto StereoPi
STEREOPI_BASE = Path("/home/pi/stereopi-fisheye-robot-master")
...
# Resolución de procesamiento - DEBE coincidir con la calibración existente
# Por defecto el script de calibración usa 320x240
IMG_WIDTH = 320
IMG_HEIGHT = 240
SCALE_RATIO = IMG_WIDTH / (CAM_WIDTH / 2)  # Dividir entre 2 porque es estéreo side-by-side
...
NUM_DISPARITIES = 160  # Debe ser múltiplo de 16
BLOCK_SIZE = 3
WLS_LAMBDA = 4000.0
WLS_SIGMA = 0.8
...
def load_calibration() -> Tuple[np.ndarray, ...]:
    """Carga los datos de calibración de la cámara."""
    # El archivo de calibración usa la altura de procesamiento
    calib_path = CALIBRATION_DIR / f"{IMG_HEIGHT}p" / "stereo_camera_calibration.npz"
    ...
    if not calib_path.is_file():
        ...
        logger.error("Asegúrate que img_height=%d en el script de calibración", IMG_HEIGHT)
    ...
    logger.info("Calibración cargada correctamente para tamaño: %s", tuple(data['imageSize']))
...
def split_and_rectify(
    ...
    # Escalar a la resolución de procesamiento
    img_width = IMG_WIDTH
    img_height = IMG_HEIGHT
    left_color_scaled = cv2.resize(left_color, (img_width, img_height), interpolation=cv2.INTER_AREA)
    right_color_scaled = cv2.resize(right_color, (img_width, img_height), interpolation=cv2.INTER_AREA)
    
    # Convertir a escala de grises
    left_gray = cv2.cvtColor(left_color_scaled, cv2.COLOR_BGR2GRAY)
    right_gray = cv2.cvtColor(right_color_scaled, cv2.COLOR_BGR2GRAY)
    ...
    left_rectified_color = cv2.remap(
        left_color_scaled, left_map_x, left_map_y, interpolation=cv2.INTER_LINEAR
    )
    
    logger.info("Rectificación completada")
    return left_rectified, right_rectified, left_rectified_color
...
def main() -> None:
    ...
    try:
        ...
        # Cargar calibración
        left_map_x, left_map_y, right_map_x, right_map_y, qq = load_calibration()
        
        # Capturar frames
        frame = capture_stereo_frames(ACCUMULATED_FRAMES)
        
        # Procesar imágenes
        left_rectified, right_rectified, left_rectified_color = split_and_rectify(
            frame, left_map_x, left_map_y, right_map_x, right_map_y
        )
